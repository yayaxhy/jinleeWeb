...
export const OPENING_TWO_ORDER_TARGET = 2;
export const OPENING_QUALIFYING_TRANSACTION_MIN_AMOUNT = 100;
...
const localDateParts = (date: Date): DateParts => {
  const values = berlinDateFormatter
    .formatToParts(date)
...
export const getBerlinWeekKey = (now = new Date()) =>
  getBerlinDateKey(getBerlinWeekStart(now));

export const getBerlinWeekEnd = (now = new Date()) => {
  const parts = localDateParts(getBerlinWeekStart(now));
  const nextMonday = new Date(
    Date.UTC(parts.year, parts.month - 1, parts.day + 7),
  );
  return berlinLocalTime(
