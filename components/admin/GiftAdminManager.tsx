...
export function GiftAdminManager({ initialGifts, endpoint }: Props) {
  ...
  const handleUpdate = async (giftName: string) => {
    ...
    try {
      ...
      setGifts((prev) => sortByName(prev.map((gift) => (gift.name === giftName ? data.gift! : gift))));
      setFiles((prev) => ({ ...prev, [giftName]: null }));
      setInputKeys((prev) => ({ ...prev, [giftName]: (prev[giftName] ?? 0) + 1 }));
      setMessages((prev) => ({ ...prev, [giftName]: '已更新' }));
    } catch (err) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCreateForm({ name: '', price: '', category: '默认', active: true, file: null });
