...
const ensureReadSession = async () => {
  const session = await getServerSession();
  if (!session?.discordId || !canViewReferrals(session.discordId)) return null;
  return session;
};
...
export async function GET() {
  const session = await ensureReadSession();
  if (!session) {
    return NextResponse.json({ error: '无权访问' }, { status: 403 });
  }

  const policies = await prisma.referralPolicy.findMany({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const referralType = parseReferralType(body?.referralType);
    ...
    if (!inviterId) {
      return NextResponse.json({ error: 'inviterId 为必填' }, { status: 400 });
    }
    if (rate === null) {
      return NextResponse.json({ error: '返利比例为必填' }, { status: 400 });
    }
    ...
    const created = await prisma.referralPolicy.create({
      data: {
        inviterId,
        referralType,
        rate,
