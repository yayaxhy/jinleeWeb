...
import { isHowardDiscordId } from '@/lib/admin';
...
export default async function HowardLayout({ children }: { children: ReactNode }) {
  const session = await getServerSession();
  if (!session?.discordId || !isHowardDiscordId(session.discordId)) {
    redirect('/');
  }
    ...
    <section className="min-h-screen bg-[#020204] text-white">
      <div className="admin-shell mx-auto flex max-w-[1400px] flex-col gap-10 px-6 py-12">
        <header className="space-y-2">
          ...
          <h1 className="text-3xl font-semibold">Howard 工作台</h1>
