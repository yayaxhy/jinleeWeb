  ...
  IF to_regclass('"PEIWAN"') IS NOT NULL
    ...
    BEGIN
      IF TG_OP = 'INSERT'
         ...
         OR NEW."baseCommissionRate" IS DISTINCT FROM OLD."baseCommissionRate" THEN
        UPDATE "Member"
           SET "commissionRate" = NEW."commissionRate",
               "baseCommissionRate" = NEW."baseCommissionRate"
    ...
    EXECUTE '
      ...
      AFTER INSERT OR UPDATE OF "commissionRate", "baseCommissionRate" ON "PEIWAN"
      FOR EACH ROW
      EXECUTE FUNCTION public.sync_peiwan_commission_to_member()
    ';

    UPDATE "Member" m
       SET "commissionRate" = p."commissionRate",
           "baseCommissionRate" = p."baseCommissionRate"
      FROM "PEIWAN" p
     WHERE m."discordUserId" = p."discordUserId"
       AND (
         m."commissionRate" IS DISTINCT FROM p."commissionRate"
         OR m."baseCommissionRate" IS DISTINCT FROM p."baseCommissionRate"
       );
  END IF;
END
$block$;
